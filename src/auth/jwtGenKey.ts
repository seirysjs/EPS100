...
const jwtSecretKeyGen = (() =>
  `4${generateHexRandom}S${generateStringRandom}E${generateMathRandom}C${substringTime}R${generateMathRandom}E${generateStringRandom}T${generateStringRandom}Key${generateHexRandom}2`)();

const envNodeJs = process.env.NODE_ENV; // Gen switches to static in !=prod ENV

const jwtSecretKey = {
  runtimeSecretKey: envNodeJs == 'production' ? jwtSecretKeyGen : 'devMode',
};
