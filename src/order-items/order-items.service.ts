...
export class OrderItemsService {
  ...
  async fulfillOrderItem(
    orderItemFulfill: OrderItemFulfill,
  ...
  async substractOrderItems(orderItemsFulfillCounts): Promise<object> {
    for (const [orderItemId, fulfillAmount] of Object.entries(
      orderItemsFulfillCounts,
    )) {
      ...
      if (orderItem.count == 0) await this.remove(orderItem.order_item_id);
    }
  ...
  async findAllFulfilledByOrder(orderId: number): Promise<object> {
    const orderItemFulfills = await this.orderItemFulfillsRepository.find({
      ...
      where: { order_id: orderId },
      order: { order_item_fulfill_id: 'DESC' },
    });
    const transferItems = await this.transferItemsRepository.find({
      relations: [
        'transfer',
        'transfer.order',
        'transfer.order.order_items',
        'transfer.order.client',
        'transfer.order.transport',
        'blueprint',
        'blueprint.product_class',
        'blueprint.product_size',
      ],
    ...
    for (
      let orderItemFulfillIndex = 0;
      orderItemFulfillIndex < orderItemFulfills.length;
      orderItemFulfillIndex++
    ) {
      const orderItemFulfill = orderItemFulfills[orderItemFulfillIndex];
      if (!sortedByBlueprint[orderItemFulfill.blueprint_id])
        sortedByBlueprint[orderItemFulfill.blueprint_id] = {
          count: 0,
          orderItemFulfills: [],
        };
      sortedByBlueprint[orderItemFulfill.blueprint_id].count +=
        orderItemFulfill.count;
      sortedByBlueprint[orderItemFulfill.blueprint_id].orderItemFulfills.push(
        orderItemFulfill,
      );
    }
    ...
    for (
      ...
      transferItemIndex < transferItems.length;
      transferItemIndex++
    ) {
  ...
  async findAllForBillByOrder(orderId: number): Promise<object> {
    const orderItemFulfills = await this.orderItemFulfillsRepository.find({
      relations: [
        'order',
        'order.bills',
        'order.bills.bill_items',
        'order.order_items',
        'order.order_item_fulfills',
        'blueprint',
        'blueprint.product_class',
        'blueprint.product_size',
      ],
    ...
    const billItems = await this.billItemsRepository.find({
      relations: [
        'bill',
        'bill.order',
        'bill.order.order_items',
        'bill.order.order_item_fulfills',
        'blueprint',
        'blueprint.product_class',
        'blueprint.product_size',
      ],
    ...
    for (
      let billItemIndex = 0;
      billItemIndex < billItems.length;
      billItemIndex++
    ) {
  ...
  async findAllFulfilledByOrderItem(
    ...
    return await this.orderItemFulfillsRepository.find({
      ...
      where: {
        ...
        blueprint_id: orderItem.blueprint_id,
      },
      order: { order_item_fulfill_id: 'DESC' },
    });
  }
  ...
  async findAllOrderItemsWithFulfillsByBlueprint(
    ...
    orderItems.forEach((orderItem) => {
      if (!sortByOrderIds[orderItem.order_id])
        sortByOrderIds[orderItem.order_id] = {};
      if (!sortByOrderIds[orderItem.order_id][orderItem.blueprint_id])
        sortByOrderIds[orderItem.order_id][orderItem.blueprint_id] = 0;
      sortByOrderIds[orderItem.order_id][orderItem.blueprint_id] +=
        orderItem.count;
    });
  ...
  async findAllWIPByBlueprint(blueprintId: number): Promise<OrderItem[]> {
    return await this.orderItemsRepository.find({
      ...
      where: { blueprint_id: blueprintId, order: { status: 'wip' } },
      order: { order_item_id: 'DESC' },
    });
  }
