...
export class BlueprintsController {
  ...
  async create(@Body() blueprint: Blueprint): Promise<object> {
    ...
    if (!blueprint.product_class_id) content.errors.push({
      property: 'product_class_id',
      constraints: {
        'is-empty': `product_class_id must be selected`,
      },
    });
    if (!blueprint.product_size_id) content.errors.push({
      property: 'product_size_id',
      constraints: {
        'is-empty': `product_size_id must be selected`,
      },
    });
    if (!blueprint.product_class_id || !blueprint.product_size_id) return content;
    const checkUninque = await this.blueprintsService.findByProductClassAndSize(blueprint.product_class_id, blueprint.product_size_id);
  ...
  @UseGuards(JwtAuthGuard)
  @Post(':id/edit')
  async update(@Body() blueprint: Blueprint, @Param('id') id: number): Promise<Blueprint> {
