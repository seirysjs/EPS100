...
export class BlocksController {
  ...
  @UseGuards(JwtAuthGuard)
  @Post('multi-cut')
  async blockMultiCut(@Body() formData: any): Promise<object> {
    const content = {
      errors: [],
    };
    ...
    for (let indexBlock = 0; indexBlock < blocks.length; indexBlock++) {
      ...
      if ((await this.blocksService.findOne(block.block_id)).status != 'done')
        continue;
      content.errors.push({
        property: 'block_' + block.block_id,
        constraints: [
          {
            ['block_' + block.block_id]: `block_${block.block_id} already done`,
          },
        ],
      });
    }
    ...
    blockMultiCut.block_multi_cut_id =
      (await this.blocksService.getLastBlockMultiCutId()) + 1;
  ...
  async processForm(
    ...
    for (let selectInput = 0; selectInput < cuts.length; selectInput++) {
      ...
      warehouseItem.count = partsCount;
      warehouseItem.created_at = new Date();
      warehouseItem.worker_id = (
        await this.blocksService.findOne(id)
      ).worker_id;
  ...
  @UseGuards(JwtAuthGuard)
  @Get('by-class/:id')
  async findAllByClass(@Param('id') id: number): Promise<Block[]> {
  ...
  async findAllByClassDrying(@Param('id') id: number): Promise<Block[]> {
  ...
  async findAllByClassQueue(@Param('id') id: number): Promise<Block[]> {
  ...
  @UseGuards(JwtAuthGuard)
  @Get('by-worker/:id')
  async findAllByWorker(@Param('id') id: number): Promise<Block[]> {
  ...
  async findAllByStatus(@Param('status') status: string): Promise<Block[]> {
  ...
  async getCutsByBlock(@Param('id') id: number): Promise<BlockCut[]> {
  ...
  async getCutsByWorker(@Param('id') id: number): Promise<BlockCut[]> {
  ...
  async getMultiCutByBlock(@Param('id') id: number): Promise<BlockMultiCut[]> {
