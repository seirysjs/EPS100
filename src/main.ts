...
async function bootstrap() {
  ...
  hbs.registerHelper("orderDeliveryDate", datetimeFormat => (datetimeFormat) ? datetimeFormat.toLocaleString('lt-LT').split(" ")[0] : new Date().toLocaleString('lt-LT').split(' ')[0]);
  hbs.registerHelper("orderVazDate", datetimeFormat => {
    if (!datetimeFormat) return "";
    const dateArray = datetimeFormat.toLocaleString('lt-LT', { year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: 'numeric' }).split(" ");
    dateArray.splice(3, 0, "mėn.");
    dateArray.push("val.");
    return dateArray.join(" ");
});

  hbs.registerHelper("todayDate", () => new Date().toLocaleString('lt-LT').split(' ')[0]);
  ...
  hbs.registerHelper("countVolume", warehouseItem => {
    ...
    const stackInOrder = (warehouseItem.orderStack == 0) ? null : warehouseItem.orderStack;
  ...
  hbs.registerHelper("floatAmountToWordsLt", (inputValue) => { 
    ...
    if (!billItems || !priceList) return amountProductTotalPrice + " " + ltEurSecondDecimalWRest;
    if (billItems.length == 0 || priceList.prices.length == 0) return amountProductTotalPrice + " " + ltEurSecondDecimalWRest;
    ...
    const totalAmountWithVAT = Math.round(((amountProductTotalPrice*21/100)+(amountProductTotalPrice)) * 100) / 100;
    ...
    const totalAmountWithoutCentsReversedSplit = totalAmountWithVAT.toString().split(".")[0].split("").reverse();
    const amountNumberDecimals = totalAmountWithoutCentsReversedSplit.length;
    const amountWords = [];
    const numberCommas = (amountNumberDecimals - amountNumberDecimals % 3)/3 + (amountNumberDecimals % 3 > 0 ? 1 : 0);

    for (let indexCommaNumber = 0; indexCommaNumber < numberCommas; indexCommaNumber++) {
      const hundreds = parseInt(totalAmountWithoutCentsReversedSplit[indexCommaNumber * 3 + 2]);
      const decimals = parseInt(totalAmountWithoutCentsReversedSplit[indexCommaNumber * 3 + 1]);
      const decimal = parseInt(totalAmountWithoutCentsReversedSplit[indexCommaNumber * 3]);

      const hundredsString = (hundreds) ? ((hundreds > 0) ? ((hundreds > 1) ? (ltFirstDecimal[hundreds] + " " + ltHundreds) : ltHundred ) : "" ) : "";
      const decimalsString = (decimals) ? ((decimals > 0) ? ((decimals > 1) ? (ltDecimals[decimals]) : ((decimal > 0) ? ltSecondDecimal[decimal] : ltDecimals[1]) ) : "") : "";
      const decimalString = (decimal) ? ((decimal > 0) ? ((decimals != 1) ? (ltFirstDecimal[decimal] + " " + ((decimal != 1) ? ltEurFirstDecimalWoOne : ltEurSingle ) ) : ltEurSecondDecimalWRest ) : "") : "";
      const commaString = (indexCommaNumber > 0) ? ((decimal != 1) ? (ltCommaMultipliers[indexCommaNumber]) : ltFirstDecimal[1] + " " + ltCommaSingleMultipliers[indexCommaNumber] ) : "";

      amountWords.unshift(hundredsString, decimalsString, decimalString, commaString);
    }
    amountWords.join("").split("");

    if (amountWords.length == 0) amountWords.push(ltZero + ltEurSecondDecimalWRest);
    if (amountWords.length > 0) amountWords.push(totalAmountWithVAT.toString().split(".")[1] ? (totalAmountWithVAT.toString().split(".")[1] + " ct") : "0 ct" );
